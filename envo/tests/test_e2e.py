...
class TestE2e:
    ...
    def test_save(self, pexpect_factory, prompt):
    ...
    def test_hot_reload(self, pexpect_factory, prompt, change_file):
    ...
    def test_hot_reload_child_dir(
        self, pexpect_factory, prompt, change_file
    ):
        p = pexpect_factory("envo --init")
        p.expect(prompt, timeout=1)
        p.sendcontrol("d")
    ...
    def test_hot_reload_error(self, pexpect_factory, prompt, change_file):
        p = pexpect_factory("envo --init")
        p.expect(prompt, timeout=1)
    ...
    def test_autodiscovery(self, pexpect_factory, prompt):
        p = pexpect_factory("envo --init")
        p.expect(prompt, timeout=1)
        p.sendcontrol("d")
    ...
    def test_init_nested(self, pexpect_factory, prompt):
